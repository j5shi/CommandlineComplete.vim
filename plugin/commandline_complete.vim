...
"                  trigger the completion, please define them in $MYVIMRC as follows:
"
...
if !hasmapto('<Plug>CmdlineCompleteBackward', 'c')
    cmap <unique> <silent> <c-k> <Plug>CmdlineCompleteBackward
endif

if !hasmapto('<Plug>CmdlineCompleteForward', 'c')
    cmap <unique> <silent> <c-j> <Plug>CmdlineCompleteForward
endif
...
try:
    ...
    vimrc_buffer    = open(vim.eval("$MYVIMRC"), 'r').readlines()
    current_buffer  = vim.current.buffer
    buffers         = [vimrc_buffer, current_buffer]
